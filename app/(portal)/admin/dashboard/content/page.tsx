...
export default async function AdminContentPage({ searchParams }: { searchParams: Promise<{ lang?: string }> }) {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-semibold text-navy">{lang === "ar" ? "إدارة المحتوى" : "Content / CMS"}</h1>

      <section className="rounded-xl border bg-white p-4">
        <h2 className="font-semibold">{lang === "ar" ? "أقسام الموقع" : "Website sections"}</h2>
          ...
          {contentKeys.map((key) => {
            ...
            return (
              <form key={key} action={upsertSiteContentAction} className="grid gap-2 rounded border p-3">
                ...
                <button className="rounded bg-navy px-3 py-2 text-sm text-white">{lang === "ar" ? "حفظ القسم" : "Save section"}</button>
              </form>
            );
          })}
        </div>
      ...
      <section className="rounded-xl border bg-white p-4">
        <h2 className="font-semibold">{lang === "ar" ? "الأسئلة الشائعة" : "FAQ entries"}</h2>
        <form action={updateFaqAction} className="mt-3 grid gap-2 rounded border p-3">
          ...
          <button className="rounded border px-3 py-2 text-sm">{lang === "ar" ? "إضافة سؤال" : "Add FAQ"}</button>
        </form>
      </section>

      <section className="rounded-xl border bg-white p-4 text-sm">
        <h2 className="font-semibold">{lang === "ar" ? "معاينة الشهادات" : "Testimonials preview"}</h2>
        {testimonials.map((t) => <div key={t.id} className="mt-2 rounded border p-2"><p className="font-medium">{t.name}</p><p className="text-slate-600">{lang === "ar" ? t.quoteAr : t.quoteEn}</p></div>)}
      </section>
    </div>
  );
}
